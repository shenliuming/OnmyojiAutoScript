...
fn emulator_view(row: EmulatorAdminRow) -> EmulatorView {
    EmulatorView {
        ...
        status: row.status,
        available: row.lifecycle_status == "READY"
            && row.occupancy_status == "IDLE"
            && row.lease_owner_type.is_none(),
        ...
        current_foster_job_id: row.current_foster_job_id,
        current_login_session_no: row.current_login_session_no,
        current_game_account_id: row.current_game_account_id,
        activity_started_at: row.activity_started_at.map(|value| value.and_utc()),
        lease_owner_type: row.lease_owner_type,
        lease_owner_key: row.lease_owner_key,
        lease_expires_at: row.lease_expires_at.map(|value| value.and_utc()),
        last_heartbeat_at: row.last_heartbeat_at.map(|value| value.and_utc()),
        bound_accounts: row.bound_accounts,
    }
}
